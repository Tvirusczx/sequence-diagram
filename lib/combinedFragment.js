...
class CombinedFragment extends DiagramElement {
    ...
    /**
     * Adds identification to each visual object part of the `CombinedFragment`.
     * This includes setting the ID, the element's class name, and the part name (e.g., 'fragmentRect', 'operatorRect').
     */
    addIdentificationToCanvasElements() {
        const className = this.constructor.name;
    
        for (let key of ['fragmentRect', 'operatorRect', 'operatorText']) {
            if (this[key]) {

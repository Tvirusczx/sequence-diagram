 ...
 */
let canvas = new fabric.Canvas('myCanvas', { preserveObjectStacking: true, selection: false,backgroundColor:'white', imageSmoothingEnabled: false });
        ...
        */
        ...
        async function importCanvasFile() {
            try {
                ...
                if (typeof jsonData === 'object' && jsonData !== null) {
                    ...
                    //alert('Canvas data imported successfully!');
                } else {
                    // Handle the case where jsonData is not valid
                    console.error('Invalid JSON data.');
                    // Display an error alert
                    alert('Invalid JSON data. Unable to import.');
                }
            } catch (error) {
                // Handle any errors that occur during the file loading or processing
                //console.error('Error importing canvas file:', error);
                // Display an error alert
                //alert('Error importing canvas file: ' + error.message + '\n Vložte validní soubor');
                alert('Vybraný soubor není validní!!! \n Vložte validní soubor');
            }
        }
        ...
        // magic handler for rerender canvas while zoom without blurry canvas objects
        // bug on resize of 100%
        /* const onResize = () => {
            ...
            const onResize = () => {
                ...
                // Uložení aktuálního nastavení pro měřítko
                const zoom = canvas.getZoom();
                
                // Resetování velikosti plátna
                ...
                // Zajištění správné pozice ovládacích prvků
                canvas.calcOffset();
            
                // Opětovné překreslení plátna
                canvas.requestRenderAll();
            };

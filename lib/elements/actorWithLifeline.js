...
/**
 * Returns editable properties for the Actor.
...
getEditableProperties() {
  ...
  /**
   * Applies the given property change to the actor.
   * @param {string} key 
   * @param {any} value 
   */
  ...
  /**
   * Helper: Gets fill color of the actor's head (Circle).
   * @returns {string}
   */
  getHeadFill() {
    const head = this.shape._objects?.find(obj => obj.type === 'circle');
    return head?.fill || '#add8e6';
  }
  
  /**
   * Helper: Sets fill color of the actor's head.
   * @param {string} color 
   */
  setHeadFill(color) {
    const head = this.shape._objects?.find(obj => obj.type === 'circle');
